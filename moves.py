...
class Moves():
    ...
    def auth(self, request_token):
        ...
        j = requests.post(
            self.auth_url + 'access_token?grant_type=authorization_code&code=' + request_token + c + s + r)
    ...
    def get_storyline(self, token, start, end, trackPoints):
        token = '&access_token=' + token
        url = self.api_url + '/user/storyline/daily?from=' + start + '&to=' + end + '&trackPoints=' + trackPoints + token
        print url
